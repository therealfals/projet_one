...
if (!empty($_GET)){
    ...
    echo "<input type='hidden' id='dbNameTxt' value='".$_GET['dbname']."'><h3 class='text-center'>Base de donnée: ".$_GET['dbname']."<br><div><a class='btn btn-sm btn-outline-warning' href='backup.php?db=".$_GET['dbname']."&type=db'>Faire un backup</a><a class='btn btn-sm btn-outline-danger' style='margin-left: 5px !important;' href='list_backup.php?db=".$_GET['dbname']."&type=one'>Voir les backup</a></div></h3>";
...
<select  class='form-control' name='tb2'>";
 ...
 <div class='overflow-auto mt-2' style='height: 350px'><table class='table table-striped'><thead> <th>Nom table</th><th>Actions</th> </thead><tbody>";
    ...
    foreach($result as $rows ){
        $rows=array_values($rows);
        if( isset($rows[0])){
            echo "<tr><td><input name='tb[]' type='checkbox' class='backup' value='".$rows[0]."'>&nbsp;". $rows[0]."</td><td><a class='btn btn-sm btn-outline-danger ml-1' href='list_backup.php?table=$rows[0]&db=".$_GET['dbname']."&type=table'>Voir les backup</a>&nbsp;</td></tr>";
        }
